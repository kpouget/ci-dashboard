...
func populateTestFromToolboxLogs(test *v1.TestResult, toolbox_logs map[string]artifacts.JsonArray) error {
	...
	for toolbox_step_name, toolbox_step_json := range toolbox_logs {
		...
		test.Ok = test.Ok + ok
		test.Failures = test.Failures + failures
		test.Ignored = test.Ignored + ignored
	}
		...
		for test_group, tests := range test_matrix.Tests {
			for test_idx := range tests {
				...
				for _, old_test_build_id := range test_build_ids {
					...
					old_test_gpu_burn_logs, err := artifacts.FetchGPUBurnLogs(&test_matrix, &old_test, old_test_build_id)

					if err != nil {
						log.Warningf("Failed to fetch the GPU burn logs of the test %s/%s: %v", test.ProwName, old_test_build_id, err)
					}

					if old_test_gpu_burn_logs == ""{
						log.Warningf("Could not find GPU burn logs for the test %s/%s", test.ProwName, old_test_build_id)
